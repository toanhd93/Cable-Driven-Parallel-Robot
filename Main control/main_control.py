...
# ====Load Model =====================
#new_model = load_model("Model_No_Tension.h5")                  # Load RNN model ver. 1

new_model = load_model("Model_No_Tension_RNN.h5")               # Load RNN model ver.2
    
#new_model = load_model("Model_No_Tension_BiRNN.h5")            # Load Bidirectional RNN model
...
for i in range(1, len(data)+1):
    ...
    if i != len(data):
        ...
        RNN_list = np.append(RNN_list, ([[X_Err, Y_Err, Z_Err]]), axis = 0)         #T1, T2, T3, T4, T5, T6, T7, T8, 
        ...
        Err_x =  Prediction[0,0]
        Err_y =  Prediction[0,1]
        Err_z =  Prediction[0,2]
